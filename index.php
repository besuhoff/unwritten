  ...
  <body>
    ...
    <script>
      var data = [
        ...
        {
          content: '<p>…вошел уже в тот возраст, когда знакомые чуть старше него вырастили дочерей, за которых если бы вздумалось ему выйти замуж, то общество уж далеко не сочло бы его педофилом, а лишь человеком слегка взбалмошным и эксцентричным.</p>',
          tags: ['из_ненаписанного', 'записки_анонимного_невротика']
        },
        {
          content: '<p>Однажды тротуары таки начнут делать из сверхгидрофобной плитки, и в этот славный день, выйдя на улицу, какой-нибудь нечестивый пенсионер вроде меня, понося современную молодежь, как харкнет смачно наземь, а плевок как отскочит обратно и прямо в глаз ему! Так-то.</p>',
          tags: ['из_ненаписанного', 'хроники_коммунальной_квартиры']
        },
        {
          content: '<p>&mdash; Блин, опять джинсы твои износились, смотри, растянутые все, пузыри вон на коленях!</p><p>&mdash; Ну маам, ты не понимаешь, это не пузыри, а о-вер-сайз!</p>',
          tags: ['из_ненаписанного', 'хроники_коммунальной_квартиры']
        },
        {
          content: '<p>Володя был у нас, по фамилии Данченко, и наши его в шутку прозвали Немирович, а он по доброте душевной своей и наивности думал, что они его подкалывают так, что он, мол, бухает всё время, пока кто-то не рассказал ему, наконец, историю Художественного театра</p>',
          tags: ['из_ненаписанного', 'котлован']
        }
      ]
    </script>
    ...
    <script>
      ...
      function syncUrlState(url, push) {
        ...
        if (push === undefined || push) {
          history.pushState(state, document.querySelector('title').innerText, renderStateIntoUrl());
        }
      ...
      window.onpopstate = function(event) {
        if (event.state) {
          syncUrlState(renderStateIntoUrl(event.state.tag, event.state.index), false);
          renderTemplate('main');
          initNavigation();
        }
      };
